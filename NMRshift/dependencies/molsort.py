...
cfmid_input_2D_nmr.txt: written by predict_sdf.bat by redirection of the standard output
cfmid_input_2D_nmr_sorted.txt: read by cfmid_input_2D_nmr_sorted.txt

called by: python molsort.py

Each line in file cfmid_input_2D_nmr.txt contains a molecule index (start at 0),
a carbon atom index (start at 0) and the corresponding 13C NMR chemical shift, all separated by a space.
Molecules are indexed according to their position in file.
Carbon atoms are indexed according to their position in the MolBlock in file.
...
molsort.py reorders carbon atoms in molecules and molecules in file fcfmid_input_2D_nmr.txt
to produce file cfmid_input_2D_nmr_sorted.txt.
File cfmid_input_2D_nmr_sorted.txt contains one line per compound, made of 3 ' | '-separated fields:
...
# define input and output files
	inputfilename = 'cfmid_input_2D_nmr.txt'
	outputfilename = 'cfmid_input_2D_nmr_sorted.txt'
# build output file
...
# write formatted output to file cfmid_input_2D_nmr_sorted.txt
